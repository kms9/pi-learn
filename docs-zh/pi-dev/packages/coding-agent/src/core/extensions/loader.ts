...
// 静态导入扩展可能使用的包。
// 必须是静态导入，这样 Bun 才会把它们打进编译后的二进制。
// 随后通过 virtualModules 选项把这些包提供给扩展。
import * as _bundledTypebox from "typebox";
import * as _bundledTypeboxCompile from "typebox/compile";
import * as _bundledTypeboxValue from "typebox/value";
import { CONFIG_DIR_NAME, getAgentDir, isBunBinary, isBundledNode } from "../../config.ts";
// 注意：此导入之所以可行，是因为 loader.ts 的导出不会从 index.ts 再导出，
// 从而避免循环依赖。扩展可以从 @earendil-works/pi-coding-agent 导入。
...
/** 通过 virtualModules 提供给扩展的模块（用于编译后的二进制） */
const VIRTUAL_MODULES: Record<string, unknown> = {
	...
	// 扩展把 pi-ai 根路径解析到 compat 入口（core 入口的严格超集）：
	// 使用旧全局 API 的现有扩展在运行时仍可工作，直到 compat 被移除。
...
/**
 * 获取 jiti 的别名（用于已构建的 Node.js 模式）。
 * 编译二进制模式下改用 virtualModules。
 */
...
/**
 * 创建运行时：action 方法先是会抛错的桩实现。
 * Runner.bindCore() 会把它们替换成真正的实现。
 */
export function createExtensionRuntime(): ExtensionRuntime {
	...
	const runtime: ExtensionRuntime = {
		...
		// registerTool() 在扩展加载期间即可调用；refresh 仅在 bind 之后才需要。
		refreshTools: () => {},
		...
		// bind 之前：把注册排入队列，等 bindCore() 在 model registry 可用后一次性 flush。
		// bindCore() 会把这两个方法替换成直接调用。
...
function createExtensionAPI(
	...
	const api = {
		...
		// Flag 访问：检查该扩展是否注册过，再从 runtime 读取
...
async function loadExtensionModule(extensionPath: string, cacheToken?: ExtensionCacheToken) {
	...
	const jiti = createJiti(import.meta.url, {
		moduleCache: false,
		// 编译后的二进制和打包的 Node 发行版使用内嵌模块。
		// 源码 TypeScript 复用宿主模块和根 tsconfig paths。未打包的
		// Node 构建使用 dist 别名。
 ...
 *
 * 检查顺序：
 * 1. 带 "pi.extensions" 字段的 package.json -> 返回声明的路径
 * 2. index.ts 或 index.js -> 返回该 index 文件
 *
 * 找到入口则返回已解析路径，否则返回 null。
 */
function resolveExtensionEntries(dir: string): string[] | null {
	// 先检查带 "pi" 字段的 package.json
 ...
 * 2. 带子目录 index：`extensions/* /index.ts` 或 `index.js` → 加载
 * 3. 带子目录 package.json：`extensions/* /package.json` 且含 "pi" 字段 → 加载其声明的入口
 *
 * 只向下递归一层。复杂包必须用 package.json manifest。
 */
...
export async function discoverAndLoadExtensions(
	...
	for (const p of configuredPaths) {
		...
		if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
			// 检查带 pi manifest 的 package.json 或 index.ts
