...
// 扩展快捷键会与 keybindings.json 中的规范 keybinding id 冲突。
// 这里只保留编辑器全局快捷键。选择器专用绑定不在此列。
...
/** 所有 before_agent_start handler 的合并结果 */
...
/**
 * 由通用 emit() 方法处理的事件。
 * 有专用 emitXxx() 方法的事件被排除，以获得更强的类型安全。
 */
 ...
 * 发出了事件返回 true，没有 handler 则返回 false。
 */
...
export async function emitProjectTrustEvent(
	...
	for (const ext of extensionsResult.extensions) {
		// 单个扩展可以为同一事件注册多个 handler。
		// 第一个返回 yes/no 的 project_trust handler 胜出；undecided 则继续往下。
...
export class ExtensionRunner {
	...
	bindCore(
		...
		// 把 actions 复制进共享 runtime（所有扩展 API 都引用它）
	...
	/**
	 * 请求优雅关闭。由扩展工具和事件 handler 调用。
	 * 实际关闭行为由各 mode 通过 bindExtensions() 提供。
	 */
	shutdown(): void {
	...
	/**
	 * 创建供事件 handler 和工具执行使用的 ExtensionContext。
	 * 上下文值在调用时解析，因此 bindCore/bindUI 的变更会立即反映。
	 */
	...
	createCommandContext(): ExtensionCommandContext {
		// 使用属性描述符而不是对象展开，让 createContext() 里带守卫的 getter 保持惰性。
		// 展开会立刻读一次并把旧值冻进返回对象，从而绕过过期实例检查。
	...
	/** 发出 input 事件。transform 会串联，"handled" 会短路。 */
	async emitInput(
		text: string,
		images: ImageContent[] | undefined,
		source: InputSource,
