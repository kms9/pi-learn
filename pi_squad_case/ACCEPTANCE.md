...
本目录 6 个阶段当前共 **104 个计划用例**，均尚未执行：ID 15、DISC 16、MSG 12、INV 15、TEAM 33、OBS 13。用例不是可运行脚本；文中的 lab 工具也需要先实现。阶段文档中的验收矩阵是各阶段权威清单；后续增删用例时必须同步更新本段统计。该文档规定实施后怎样证明功能可用，以及哪些证据不足以宣告通过。
...
104 个用例分组为 ID 15（原9＋ID-X01—06）、DISC 16（原12＋DISC-X01—04）、MSG 12、INV 15、TEAM 33、OBS 13。TEAM 组以阶段 04 `TEAM_RUNTIME_REQUIREMENTS.md` 的 TR-A01—TR-A33 为准，覆盖协作闭环、跨 Team Role 竞争/等待、peer invoke 续接、上下文、故障恢复与最终 Gate。每份阶段文档定义了该组全部步骤和判据；不要用一个 lab 命令替代整组用户验收。
